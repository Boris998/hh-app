  ...
  export class ELOCalculationService {
     ...
     */
    async calculateActivityELO(activityId: string): Promise<ELOCalculationResult[]> {
      ...
      try {
        ...
        const results = settings.teamBased 
          ...
          : await this.calculateIndividualELO(participantsWithSkillBonuses, settings);
        ...
        console.log(`✅ ELO calculation completed for ${results.length} participants`);
        return results;
    ...
    private async calculateIndividualELO(
      ...
      for (let i = 0; i < sortedParticipants.length; i++) {
        ...
        for (let j = 0; j < sortedParticipants.length; j++) {
          ...
          const expectedScore = this.calculateExpectedScore(player.currentELO, opponent.currentELO);
        ...
        results.push({
          ...
          skillBonus,
          reason: `Individual performance vs ${sortedParticipants.length - 1} opponents`
        });
      }
    ...
    private async calculateTeamELO(
      ...
      for (let i = 0; i < teamNames.length; i++) {
        for (let j = i + 1; j < teamNames.length; j++) {
          ...
          const actualScore1 = this.getTeamActualScore(teamResults[team1Name], teamResults[team2Name]);
          const actualScore2 = 1 - actualScore1;
          
          // Apply changes to all team members
          this.applyTeamELOChanges(
            teams[team1Name], expectedScore1, actualScore1, settings, results
          );
          this.applyTeamELOChanges(
            teams[team2Name], expectedScore2, actualScore2, settings, results
          );
        }
      }
     ...
     */
    private getKFactor(player: ParticipantELO, settings: ActivityELOSettings): number {
     ...
     */
    private getActualScore(player: ParticipantELO, opponent: ParticipantELO): number {
      if (player.finalResult === 'win' && opponent.finalResult === 'loss') return 1;
      if (player.finalResult === 'loss' && opponent.finalResult === 'win') return 0;
      if (player.finalResult === 'draw' && opponent.finalResult === 'draw') return 0.5;
    ...
    private async calculateSkillBonuses(
      ...
      for (const participant of participantsWithBonuses) {
        ...
        const historicalAverage = skillSummaries.length > 0 
          ? skillSummaries.reduce((sum, s) => sum + (s.averageRating || 0), 0) / skillSummaries.length
    ...
    private async getActivityData(activityId: string): Promise<{
      ...
      const settings: ActivityELOSettings = {
        ...
        ...activityData.activityType.defaultELOSettings as Partial<ActivityELOSettings>,
      };
      ...
      const participantsData = await db
        ...
        .leftJoin(userActivityTypeELOs, and(
          ...
          eq(userActivityTypeELOs.activityTypeId, activityData.activity.activityTypeId)
        ))
        .where(
          and(
            eq(activityParticipants.activityId, activityId),
    ...
    private groupParticipantsByTeam(participants: ParticipantELO[]): Record<string, ParticipantELO[]> {
    ...
    private calculateTeamAverageELO(teamMembers: ParticipantELO[]): number {
      const totalELO = teamMembers.reduce((sum, member) => sum + member.currentELO, 0);
      return Math.round(totalELO / teamMembers.length);
    }

    private determineTeamResults(teams: Record<string, ParticipantELO[]>): Record<string, 'win' | 'loss' | 'draw'> {
      const teamNames = Object.keys(teams);
    ...
    private sortParticipantsByPerformance(participants: ParticipantELO[]): ParticipantELO[] {
    ...
    private comparePlayerRankings(player: ParticipantELO, opponent: ParticipantELO): number {
    ...
    private async applyELOChanges(activityId: string, results: ELOCalculationResult[]): Promise<void> {
      // Apply all ELO changes in a single transaction
        ...
        for (const result of results) {
          ...
          await tx
            ...
            .values({
              ...
              activityTypeId: activityId, // Need to get from activity
              eloScore: result.newELO,
              gamesPlayed: 1, // Increment by 1
              peakELO: result.newELO, // Update if new peak
    ...
    private async completeELOCalculation(activityId: string, results: ELOCalculationResult[]): Promise<void> {
      await db
    ...
    private async handleELOCalculationError(activityId: string, error: any): Promise<void> {
      await db
        .update(activityELOStatus)
        .set({
          status: 'error',
